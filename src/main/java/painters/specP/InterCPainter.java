...
import java.awt.*;
@Getter
public class InterCPainter extends ElementPainter {

    protected Shape shape;

    public InterCPainter(DiagramElement model){
        super(model);
        //shape = new Rectangle(new Dimension(100,50));
    }
    ...
    public void draw(Graphics2D g) {
        if (model instanceof Interclass) {
            ...
            g.drawString("c", ((Interclass) model).getPosition().x + 2, ((Interclass) model).getPosition().y + 8);
            ...
            //pa printaj liniju
            cy+= 2;
            g.drawLine(((Interclass) model).getPosition().x, cy,
                    ((Interclass) model).getPosition().x + shape.getBounds().width,cy);
            cy += 3;
            //pa printaj sve metode
            for(ClassContent cc: ((Interclass) model).getContents()){
