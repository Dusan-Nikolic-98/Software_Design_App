...
import java.awt.*;
@Getter
public class InterEnumPainter extends ElementPainter {
    protected Shape shape;

    public InterEnumPainter(DiagramElement model) {
        super(model);
    }
    ...
    public void draw(Graphics2D g) {
        if (model instanceof Interclass) {
            ...
            g.drawString("e", ((Interclass) model).getPosition().x + 2, ((Interclass) model).getPosition().y + 8);
            ...
            for(ClassContent cc: ((Interclass) model).getContents()){
                if(cc instanceof EnumClassContent){
                    //printaj sve enumove enume
                    cy += strH;
